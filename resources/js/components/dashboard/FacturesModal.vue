<template>
      ...
      <div class="bg-gray-900 p-8 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-auto border border-gray-800">
        ...
        <div class="mt-6">
          ...
          >
            ...
            <p class="text-gray-400">Aucune facture !</p>
          </div>
          ...
          <div v-else class="space-y-3">
            ...
            :key="invoice.id">
                ...
                <label
                v-for="prestation in invoice.prestations"
                :key="prestation.id"
        ...
        <!-- Récapitulatif -->
        <div v-if="invoices.length !==0" class="mt-6 bg-gray-800 p-4 rounded-xl border border-gray-700">
    ...
    import { FacturePdfModal,} from '@/components/factures/'
    ...
    const totalHeures = computed(() => {
      return invoices.value
        .reduce((sum, invoice) => sum + parseFloat(invoice.heures_total), 0);
    });
    
    const totalHT = computed(() => {
      return invoices.value.reduce((total, invoice) => {
        return total + invoice.montant_total
      }, 0);
    });
